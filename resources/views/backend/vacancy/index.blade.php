                                ...
                                <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                    ...
                                    @foreach($vacancy as $item)
                                        <tr>
                                            ...
                                            <td>{{$item->opening}}</td>
                                            <td>{{$item->apply_before}}</td>

                                            <td>
                                                <a href="{{route('vacancy.edit',$item->id)}}" data-toggle="tooltip" title="edit" class="float-left btn btn-sm btn-outline-warning" data-placement="bottom"><i class="fas fa-edit"></i> </a>
                                                <form class="float-left ml-1" action="{{route('vacancy.destroy',$item->id)}}"  method="post">
                                                    @csrf
                                                    @method('delete')
